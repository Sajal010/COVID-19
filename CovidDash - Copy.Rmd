...
dataSets <-function(){
  ...
  write.csv(recovered, file = "recovered.csv")
  write.csv(vaccinated, file = "vaccinated.csv")
  write.csv(vaccineDose, file = "vaccineDose.csv")
  
  flag = 1
  
  if(nrow(deaths) == 0 | nrow(confirmed) == 0 | nrow(confirmed == 0) | nrow(vaccinated) == 0){
    deaths <- read.csv("deaths.csv")
    confirmed <- read.csv("confirmed.csv")
    recovered <-read.csv("recovered.csv")
    vaccinated <-read.csv("vaccinated.csv")
    vaccineDose <- read.csv("vaccineDose.csv")
    flag= 2
  }
  
  return(list(deaths = deaths, confirmed = confirmed, recovered = recovered, vaccinated = vaccinated, vaccineDose = vaccineDose,flag = flag))
}

data <- dataSets()
load("JHUCountries.RData")
...
death <- death[,-c(1,2,4,5)]
confirmed <- confirmed[,-c(1,2,4,5)]
recovered <- recovered[,-c(1,2,4,5)]
vaccinated <- vaccinated[,-8]
vaccineDose <- vaccineDose[,-c(1,2,4,8)]
...
dataReactiveDeathDT <- reactive({
  df <- getDaily(data = death) %>% 
    ...
    mutate(Global = rowSums(select(., input$countrySelect2))) %>%
    mutate(DailyCount = c(Global[1], diff(Global)) )
  return(df)
})
...
2. **In the ‘Covid Infection’ tab, various logistics related to selected country/countries like no. of confirmed cases/recovered cases/deaths globally/selected countries are shown. Also, graphs of daily confirmed cases/deaths/recoveries, cumulative confirmed cases/deaths cases/recovered cases and cases/mortality by country can be seen.**
...
renderValueBox({
  ...
  valueBox("Number of Confirmed Cases", value = prettyNum(totalConfirmed ,big.mark = ","), color = "black")
})
```
...
renderValueBox({
  ...
  valueBox("Number of Recovered Cases", value = prettyNum(totalRecovered, big.mark = ","), color = "#008076",icon = "fa-heartbeat")
})
```
...
renderValueBox({
  ...
  valueBox("Number of Deaths", value = prettyNum(totalDeath ,big.mark = ",") , color = "#AA0000",icon = "fa-ambulance")
})
```
...
renderValueBox({
  ...
  valueBox("Total Number of Vaccine Administered", value = prettyNum(totalVaccine ,big.mark = ","), color = "#006800",icon = "fa-syringe")
})
```
...
renderValueBox({
  ...
  valueBox("Total Number of First Doses", value = prettyNum(firstDose, big.mark = ","), color = "#FFD505",icon = "fa-syringe")
})
```
...
renderValueBox({
  ...
  valueBox("Total Number of Second Doses", value = prettyNum(SecondDose ,big.mark = ",") , color = "#00A800",icon = "fa-syringe")
})
```
...
```{r}
renderPlotly({
  gVaccineData <- aggregate(.~Date, vaccinated[-c(1,2,7,8,9)],sum)
...
renderPlotly({
  if(input$map == "Confirmed Cases"){
    fig <- plot_geo(mapDataCases, type='choropleth', locations=mapDataCases$iso3, z=mapDataCases$Confirmed, text=mapDataCases$Country, colors="Purples") %>% 
   layout(title = paste0("<br> <br> World map showing no. of <br>",input$map)) %>% 
    colorbar(title = paste0("No. of ",input$map))
   ...
   layout(title = paste0("<br> <br> World map showing no. of <br>",input$map)) %>% 
    ...
    fig <- plot_geo(mapDataCases, type='choropleth', locations=mapDataCases$iso3, z=mapDataCases$Recovered, text=mapDataCases$Country, colors="YlGn") %>% 
   layout(title = paste0("<br> <br> World map showing no. of <br>",input$map)) %>% 
    colorbar(title = paste0("No. of ",input$map))
  }else if(input$map == "Vaccination (Total Doses)"){
    fig <- plot_geo(mapDataVaccines, type='choropleth', locations=mapDataVaccines$iso3, z=mapDataVaccines$Doses_admin, text=mapDataVaccines$Country, colors="BuGn") %>% 
   layout(title = paste0("<br> <br> World map showing no. of <br>",input$map)) %>% 
    colorbar(title = paste0("No. of ",input$map))
  }else if(input$map == "Vaccination (First Dose)"){
    fig <- plot_geo(mapDataVaccines, type='choropleth', locations=mapDataVaccines$iso3, z=mapDataVaccines$People_partially_vaccinated, text=mapDataVaccines$Country, colors="YlGn") %>% 
   layout(title = paste0("<br> <br> World map showing no. of <br>",input$map)) %>% 
    colorbar(title = paste0("No. of ",input$map))
  }else{
    fig <- plot_geo(mapDataVaccines, type='choropleth', locations=mapDataVaccines$iso3, z=mapDataVaccines$People_fully_vaccinated, text=mapDataVaccines$Country, colors="BuGn") %>% 
   layout(title = paste0("<br> <br> World map showing no. of <br>",input$map)) %>% 
    colorbar(title = paste0("No. of ",input$map))
  }
  #fig <- plot_geo(mapDataCases, type='choropleth', locations=mapDataCases$iso3, z=mapDataCases$Deaths, text=mapDataCases$Country, colorscale="Inferno")
  fig <-fig %>% config(displayModeBar = F) %>% layout(height = 950)
  fig
})
```
